...
function userLogin() {
    ...
    // 2、根据用户信息进行注册
    $.post("https://localhost:5006/api/v1/User/Login", { "UserName": username, "Password": pwd }, function (result) {
        if (result.ErrorNo == "0") {
            // 1、用户信息
...
function regLogin() {
    ...
    // 2、根据用户信息进行登录
    $.post("https://localhost:5006/api/v1/User", { "UserName": username, "Password": pwd, "UserQQ": qq, "UserPhone": tel }, function (result) {
        if (result.ErrorNo == "0") {
            alert("注册成功");
            // 1、关闭注册框
            layer.closeAll();
